...
export async function extractEntitiesFromPost({
  ...
  const task = trimWhitespaceOnAllLines(`This was posted in the UCLA Housing Group on ${updated_time}. What entities can you extract from it?
  ...
  const messages: ChatCompletionUserMessageParam[] = [{ "role": "user", "content": task }];
  const functions = [
    {
      ...
      parameters: {
        ...
        properties: {
          ...
          duration: {
            ...
            properties: {
              start: { type: 'string', description: 'when the lease starts, in the form of M/D/YYYY or M/YYYY' },
              end: { type: 'string', description: 'when the lease ends, in the form of M/D/YYYY or M/YYYY' },
            },
          },
          amenities: {
            type: 'array',
            items: {
              type: 'string',
              enum: ['parking', 'furnished', 'laundry', 'AC', 'internet', 'common areas', 'balcony', 'patio', 'yard', 'heating', 'smoke free', 'cable', 'security', 'kitchen', 'gym', 'pool', 'pets']
            },
          },
          commute_walkable: {
            type: 'string',
          ...
          post_type: {
            ...
            description: "whether the post's author is offering a lease or searching for a lease",
          },
        },
        required: ['post_type']
      },
  ...
  console.log(logMetadata, `extract finished in: ${performance.now() - t0}`, responseMessage.function_call?.arguments);
  if (!responseMessage.function_call) {
    throw new Error(JSON.stringify(responseMessage));
  }
  const res = JSON.parse(responseMessage.function_call?.arguments);
  if (res.amenities && res.commute_walkable === 'walking') res.amenities.push('walkable');

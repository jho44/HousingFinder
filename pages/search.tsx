...
export default function Home({ allPosts }: { allPosts: Post[] }) {
  ...
  useEffect(() => {
    ...
    const highPriceNum = debouncedHighPrice ? parseInt(debouncedHighPrice, 10) : Infinity;
    allPosts.forEach((post) => {
      if (
        (searchType === 'searching_for' && post.post_type !== 'searching_for_lease') ||
      ...
      if (post.duration?.start && moveInDate) {
        ...
        if (postMoveInDate && !isWithin16Days(moveInDate, postMoveInDate)) return;
      }
      if (post.duration?.end && moveOutDate) {
        const postMoveOutDate = convertToDate(post.duration?.end, true);
        if (postMoveOutDate && !isWithin16Days(moveOutDate, postMoveOutDate)) return;
      }
  ...
  }, [searchType, debouncedLowPrice, debouncedHighPrice, moveInDate, moveOutDate])
      ...
      <div className="flex flex-col items-center px-4 pb-4 gap-4 pt-[calc(var(--bar-height)+16px)]">
      {
        filteredPosts.length ?
          filteredPosts.map((post) => {
            ...
            return (
              <div key={post.id} className="flex flex-col gap-3 py-3 px-4 rounded-lg max-w-[680px] bg-dark-900">
                <div className="flex justify-between items-center">
                  {/* "flex flex-row-reverse gap-2.5 " */}
                  <a href={ogPostLink} className="font-bold w-fit" target="_blank">
                    Original post
                  </a>
                  <a href={ogPostLink} className="w-fit text-sm text-dark-300" target="_blank">
                    {
